...
for i = 1:m
  ...
  a3 = ones(1, num_labels);
  a3 = sigmoid(z3);

  J += -log(a3(y(i))) - sum(log(1 - a3)) + log(1 - a3(y(i)));
%  J += -log(h(i, y(i))) - sum(log(1 - h(i, :))) + log(1 - h(i, y(i)));
...
J += lambda / 2 * (trace(Theta1(:, 2:end) * Theta1(:, 2:end)') + trace(Theta2(:, 2:end) * Theta2(:, 2:end)'));
